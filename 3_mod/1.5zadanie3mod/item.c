...
{
    struct Item *item = calloc(1, sizeof(item_t));
    ssize_t ret;
    if (item == NULL)
    {
    ...
    while(ret = read(fd, &item->info_all, sizeof(info_all_t)) > 0)
    {
...
struct Item* writing(struct Item* head, int fd)
{
    while(head != NULL)
    {
        if (write(fd, &head->info_all, sizeof(info_all_t)) == -1)
        {
...
struct Item* add(struct Item* head)
{
    ...
    do
    {
        if (compare_person(&new_item->info_all.person, &tmp->info_all.person) == -1)
        {
