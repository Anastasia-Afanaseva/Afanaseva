...
int main(int argc, char* argv[])
{
    ...
    struct sockaddr_in cliaddr_1, cliaddr_2; /* Структуры для адресов клиентов*/
    /* Сначала проверяем наличие второго аргумента в
    командной строке. При его отсутствии ругаемся и прекращаем работу */

    if (argc < 3)
    {
        printf("\nВведите номера портов свой и собеседника");
        exit(1);
    }
    ...
    cliaddr_1.sin_port = htons(atoi(argv[1]));
    ...
    cliaddr_2.sin_port = htons(atoi(argv[2]));
